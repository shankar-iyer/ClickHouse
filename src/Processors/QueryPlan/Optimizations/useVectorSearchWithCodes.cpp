...
/// "fastknn codes-as-column" optimization.
///
/// This is the brute-force counterpart of the vector-similarity-index optimization in useVectorSearch.cpp. It targets
/// tables that do NOT have a vector similarity index but DO have a self-describing quantized-codes column, i.e. a
/// MATERIALIZED column whose default expression is `fastknnEncode(vec, method, dimensions, bits)`. The codes are tiny
/// (e.g. 12 bytes) compared to the full-precision vector (e.g. 960 * 4 bytes), so we can rank cheaply over the codes
/// first and only read the heavy vector for the survivors.
///
...
///               ExpressionStep (_approx := fastknnDistance(code, ref, ...))   [approximate, codes only]
///                 (FilterStep)                                 [the WHERE/PREWHERE predicate]
///                   ReadFromMergeTree (..., code)              [reads the small codes column]
///
...
UInt64 literalToUInt64(const ASTLiteral & literal)
{
    return static_cast<UInt64>(literal.value.safeGet<UInt64>());
}

/// If `column` is a MATERIALIZED column defined as `fastknnEncode(search_column, 'method', dim, bits)`, fill the
/// out-params and return true.
bool matchFastKnnEncodeColumn(
    const ColumnDescription & column,
    const String & search_column,
    String & method_out,
    UInt64 & dimensions_out,
    UInt64 & bits_out)
{
    if (column.default_desc.kind != ColumnDefaultKind::Materialized || !column.default_desc.expression)
        return false;

    const auto * function = column.default_desc.expression->as<ASTFunction>();
    if (!function || function->name != "fastknnEncode" || !function->arguments)
        return false;
    ...
    const auto * vec_identifier = arguments[0]->as<ASTIdentifier>();
    if (!vec_identifier || vec_identifier->name() != search_column)
        return false;

    const auto * method_literal = arguments[1]->as<ASTLiteral>();
    const auto * dimensions_literal = arguments[2]->as<ASTLiteral>();
    const auto * bits_literal = arguments[3]->as<ASTLiteral>();
    if (!method_literal || method_literal->value.getType() != Field::Types::String)
        return false;
    if (!dimensions_literal || !bits_literal)
        return false;

    method_out = method_literal->value.safeGet<String>();
    dimensions_out = literalToUInt64(*dimensions_literal);
    bits_out = literalToUInt64(*bits_literal);
    return true;
}

}

bool optimizeVectorSearchWithCodes(
    ...
    const Optimization::ExtraSettings & settings, size_t max_limit_for_lazy_materialization)
{
    ...
    /// Find a self-describing codes column: MATERIALIZED <col> = fastknnEncode(search_column, method, dim, bits).
    String codes_column;
    String method;
    UInt64 dimensions = 0;
    UInt64 bits = 0;
    ...
    {
        if (matchFastKnnEncodeColumn(column, search_column, method, dimensions, bits))
        {
            codes_column = column.name;
            break;
        }
    }

    if (codes_column.empty())
        return false;
    ...
    const auto & reference_node = approx_dag.addColumn(
        reference_type->createColumnConst(1, Field(reference_field_array)), reference_type, "__fastknn_reference");

    auto string_type = std::make_shared<DataTypeString>();
    const auto & method_node = approx_dag.addColumn(
        string_type->createColumnConst(1, Field(method)), string_type, "__fastknn_method");

    auto uint64_type = std::make_shared<DataTypeUInt64>();
    const auto & dimensions_node = approx_dag.addColumn(
        uint64_type->createColumnConst(1, Field(dimensions)), uint64_type, "__fastknn_dimensions");
    const auto & bits_node = approx_dag.addColumn(
        uint64_type->createColumnConst(1, Field(bits)), uint64_type, "__fastknn_bits");

    auto uint8_type = std::make_shared<DataTypeUInt8>();
    const auto & is_l2_node = approx_dag.addColumn(
        uint8_type->createColumnConst(1, Field(static_cast<UInt64>(is_l2 ? 1 : 0))), uint8_type, "__fastknn_is_l2");

    auto fastknn_distance = FunctionFactory::instance().get("fastknnDistance", context);
    ActionsDAG::NodeRawConstPtrs distance_arguments{
        &code_node, &reference_node, &method_node, &dimensions_node, &bits_node, &is_l2_node};
    const auto & approx_node = approx_dag.addFunction(fastknn_distance, std::move(distance_arguments), approx_column_name);
    ...
    inner_sorting_node.step->setStepDescription("fastknn shortlist sort");
