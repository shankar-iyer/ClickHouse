    ...
    const QueryPlanOptimizationSettings & optimization_settings, QueryPlan::Node & root, QueryPlan::Nodes & nodes, QueryPlan & query_plan)
{
    ...
    /// fastknn "codes-as-column" optimization: for tables without a vector similarity index but with a self-describing
    /// `fastknnEncode(...)` codes column, rewrite ORDER BY distance LIMIT into a two-stage shortlist-then-rescore. It
    /// must run before lazy materialization so that the latter defers the heavy vector column on the inner shortlist.
    if (optimization_settings.try_use_vector_search)
    {
        ...
        while (!stack.empty())
        {
            ...
            if (frame.next_child == 0)
            {
                if (optimizeVectorSearchWithCodes(root, stack, nodes, extra_settings, optimization_settings.max_limit_for_lazy_materialization))
                    break;
            }
