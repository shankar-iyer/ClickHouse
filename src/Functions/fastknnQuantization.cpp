...
#include <Common/FastKnnQuantization.h>
...
/// fastknnEncode(vec, method, dimensions, bits) -> FixedString(bytesPerVector)
///
/// Stateless per-row quantization of `vec` using one of the data-independent fastknn methods. Designed to back a
/// MATERIALIZED column so the codes are produced on INSERT/MERGE and an approximate distance can be computed at query
/// time with `fastknnDistance`.
class FunctionFastKnnEncode : public IFunction
{
public:
    static constexpr auto name = "fastknnEncode";
    static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionFastKnnEncode>(); }
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        if (!FastKnnQuantization::isSupportedMethod(method))
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown fastknn quantization method '{}'", method);

        const UInt64 dimensions = getConstUIntArgument(arguments[2], name, 2);
        const UInt64 bits = getConstUIntArgument(arguments[3], name, 3);
        if (dimensions == 0)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "fastknn dimensions must be greater than zero");

        const size_t bytes = FastKnnQuantization::bytesPerVector(method, dimensions, bits);
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const override
    {
        ...
        for (size_t row = 0; row < input_rows_count; ++row)
        {
            ...
            FastKnnQuantization::encode(method, buf.data(), dimensions, bits, reinterpret_cast<char *>(&chars[row * n]));
        }
...
/// fastknnDistance(code, query, method, dimensions, bits, is_l2) -> Float32
///
/// Approximate distance between a quantized `code` (as produced by `fastknnEncode`) and the full-precision query vector
/// `query`. The query state is prepared once per call. `is_l2` selects L2Distance (1) vs cosineDistance (0).
class FunctionFastKnnDistance : public IFunction
{
public:
    static constexpr auto name = "fastknnDistance";
    static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionFastKnnDistance>(); }
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        if (!checkAndGetDataType<DataTypeFixedString>(arguments[0].type.get()))
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                "First argument of function {} must be a FixedString (a fastknn code)", name);
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        auto query = FastKnnQuantization::prepareQuery(method, query_buf.data(), dimensions, bits, is_l2);
        ...
        const size_t expected = FastKnnQuantization::bytesPerVector(method, dimensions, bits);
        if (n != expected)
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                "fastknn code has {} bytes but method '{}' with {} dimensions expects {}", n, method, dimensions, expected);
        const auto & chars = col_code->getChars();

        auto col_res = ColumnFloat32::create(input_rows_count);
        auto & res_data = col_res->getData();
        for (size_t row = 0; row < input_rows_count; ++row)
            res_data[row] = FastKnnQuantization::distance(*query, reinterpret_cast<const char *>(&chars[row * n]));
...
REGISTER_FUNCTION(FastKnnEncode)
{
...
lattice; there is no trained codebook), so the result can be produced as a `MATERIALIZED` column. Use `fastknnDistance`
to compute an approximate distance from the resulting code at query time.
...
it is ignored by the other methods.
    ...
    FunctionDocumentation::Examples examples = {
        {"Encode", "SELECT length(fastknnEncode([1., 2., 3., 4.], 'b1', 4, 0));", "1"}};
    ...
    factory.registerFunction<FunctionFastKnnEncode>(documentation);
}

REGISTER_FUNCTION(FastKnnDistance)
{
    FunctionDocumentation::Description description = R"(
Computes the approximate distance between a quantized code (as produced by `fastknnEncode`) and a full-precision query
vector. The query state is prepared once per call, so the function is efficient when scanning many codes against a single
query. `is_l2` selects `L2Distance` (1) versus `cosineDistance` (0).
)";
    ...
    FunctionDocumentation::Examples examples = {
        {"Distance", "SELECT fastknnDistance(fastknnEncode([1., 0.], 'b1', 2, 0), [1., 0.], 'b1', 2, 0, 1);", "0"}};
    FunctionDocumentation::IntroducedIn introduced_in = {25, 7};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::Distance;
    FunctionDocumentation documentation = {
        description, syntax, arguments, {}, returned_value, examples, introduced_in, category};

    factory.registerFunction<FunctionFastKnnDistance>(documentation);
}
